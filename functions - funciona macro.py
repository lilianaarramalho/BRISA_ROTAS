        ...
        if no_atual.tipo=="Nó":
            ...
            for index2 in range(len(nos)):
                ...
                if no_atual==no2:
                    ...
                    acumulado_tempo+=nos[index2].tempo
                    acumulado_kms+=nos[index2].extensao
...
def criar_rota_dividida(n_voltas, co_a_considerar, sublancos_a_considerar, id_turno,sentido_anterior,vetor_solucao):
    ...
    tempo_paragens_1=adiciona_esperas(tempo_paragens_1,id_turno,vetor_solucao)
    tempo_paragens_2 = adiciona_esperas(tempo_paragens_2, id_turno,vetor_solucao)
    ...
    if sentido_anterior=='nd':
        if (montecarlo_1 > montecarlo_2):
            rota_best = rota_2
            tempo_paragens_best = copy.deepcopy(temp_2)
            montecarlo_best=montecarlo_2
            passagem_best=copy.deepcopy(temp_2)
            sentido='crescente'
            simulacoes=output_simulacoes_2
        else:
            rota_best = rota_1
            tempo_paragens_best = copy.deepcopy(temp_1)
            passagem_best=copy.deepcopy(temp_1)
...
def gerar_montecarlo(df_probabilidades,paragens,tempo_inicio_turno,id_turno,n_voltas):
    ...
    for index in range(n_simulacoes):
        ...
        for incidencia in nos_incidencias:
            ...
            sublanco=incidencia.get('sublanço')

            tempo_sublanco_incidencia=30

            for dict in tempo_sublancos:
                if dict.get('sublanco')==sublanco:
                    tempo_sublanco_incidencia=dict.get('tempo')
            ...
            if len(pontos_hora)>0:
                ...
                posicao_procura=posicao_real_escolhida
                id_no_seguinte = id_no_partida
                while id_no_seguinte==id_no_partida:
                    posicao_procura+=1
                    id_no_seguinte=paragens[posicao_procura].get('posicao')

                tempo_inversao=0

                if id_no_seguinte>id_no_partida and id_no_chegada<id_no_partida:
                    tempo_inversao=tempos_nos_norte_sul[id_no_partida]
                elif id_no_seguinte<id_no_partida and id_no_chegada>id_no_partida:
                    tempo_inversao=tempos_nos_sul_norte[id_no_partida]

                tempo_de_resposta=get_distance(id_no_chegada,id_no_partida)+tempo_inversao
...
def adiciona_esperas(paragens,id_turno,vetor_solucao):

    global slot_tempo

    tempo_espera=(turnos[id_turno].fim-turnos[id_turno].inicio)-(paragens[-1].get('Hora Fim')-paragens[0].get('Hora Inicio'))-(sum(pausas_duracao[id_turno]))
    ...
    for combinacao in id_testes:
        ...
        delta = calcular_delta(rota,-1,vetor_solucao)
...
def verificar_corte(carro, id_turno, n_voltas):
    ...
    for index in range(len(carro) - 1):
        ...
        tempo_patrulha += get_distance(carro[index], carro[index + 1]) * n_voltas * 2
        ...
        if index==0 and carro[0]!=0:
            ...
            if index!=0:
                ...
                incidencias_consideradas.append(carro[index])
    ...
    if len(carro)>1:
        tempo_patrulha += get_distance(carro[-1], carro[-2]) * n_voltas * 2
    ...
    incidencias_consideradas.append(carro[-1])
    ...
    if (tempo_total > tempo_turno) :
        return indicadores, True

    else:
        return indicadores, False
...
def calcular_matriz_cos():
    ...
    for no in nos:
        ...
        dist_min = 99999999
        for co in lista_cos:
            distancia = get_distance(co, no.id)
